...
def display_game_over():
    game_font=pygame.font.SysFont("arialrounded",60)    #큰폰트
    txt_game_over=game_font.render(game_reslut,True,BLACK)
    rect_game_over=txt_game_over.get_rect(center=(int(screen_width/2),int(screen_height/2)))    #화면중앙에 표시
...
#게임오버 관련 변수
game_reslut=None    #게임 결과
total_time=60       #총 60초의 시간
start_ticks=pygame.time.get_ticks()     #현재 tick(시간)를 받아옴 
...
while runnig:
    ...
    for event in pygame.event.get():        
        ...
        if event.type==pygame.MOUSEBUTTONDOWN:  #마우스 버튼 누를때 
            if claw.direction !=STOP:   #집게가 좌우로 이동중일 때만 마우스이벤트 처리
    ...
    #시간 계산
    #elapsed_time=현재시간-게임시간
    elapsed_time=(pygame.time.get_ticks()-start_ticks)/1000 #ms->s    
    display_time(total_time-int(elapsed_time))   #시간표시
    #59134->5.9134->5
...
pygame.time.delay(2000)     #2초정도 대기 후 종료
pygame.quit()